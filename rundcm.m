...
%--------------------------------------------------------------------------
% Set up for node locations (MNI) for 5 different models: DMN, SAL, CEN,
% Full & BTWN (between networks)
%------------------------
% DMN
if modindex == 1
locs = {
...
%     DCM.A{forward}(Prec,lSP) = 1; %DMN-CEN Added
%     DCM.A{forward}(Prec,rSP) = 1; %DMN-CEN Added
%     DCM.A{forward}(Prec,dACC) = 1; %DMN-SAL
%     
% %     DCM.A{backward}(Prec,lSP) = 1; %DMN-CEN
% %     DCM.A{backward}(Prec,rSP) = 1; %DMN-CEN
%     DCM.A{backward}(lSP,Prec) = 1; %CEN-DMN Added
%     DCM.A{backward}(rSP,Prec) = 1; %CEN-DMN Added
%     DCM.A{backward}(dACC,Prec) = 1; %SAL-DMN
%     
% %   *************************************    
%     %** Add the intrinsic connections **%
% %   *************************************
%     
%     %Every connection is allowed to change
%     DCM.B{1} = DCM.A{1} + DCM.A{2} + DCM.A{3};
...
%     
%     DCM.A{lateral} = zeros(Nareas,Nareas);
%     DCM.A{lateral}(lSP,rSP) = 1; %lLPdmn lateral connection to rLPdmn
%     DCM.A{lateral}(rSP,lSP) = 1; %rLPdmn lateral connection to lLPdmn
%     
%     %Every connection is allowed to change
%     DCM.B{1} = DCM.A{1} + DCM.A{2} + DCM.A{3};
%     DCM.C = sparse(length(DCM.A{1}),0); %Added C so no error in PEB    
...
% DCM.options.flat     = 1; % Added for changing neural innovation to flat later                              
DCM.options.trials  = [1];   % When fitting on conditions separately
% DCM.options.trials   = [1 2];  % When fitting into a contrast
...
% %contrast vector
DCM.xU.X = [];    
% DCM.xU.X = [0 1]'; %-1]'; %1]';  % When fitting into a contrast
...
save([filepath2 datafile '_' (models{modindex}) '.mat'],'DCM')
